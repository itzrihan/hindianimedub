...
export default function Browse() {
  ...
  const { data, loading, error } = useFetch(() => api.animeList(page), [page])
  ...
  return (
    <div className={`container ${styles.page}`}>
      <SectionHeader title="All Anime" subtitle={`Page ${page}${totalPages ? ` of ${totalPages}` : ''}`} />
            ...
            {totalPages && !pages.includes(totalPages) && (
              <>
                ...
                <button onClick={() => { setPage(totalPages); window.scrollTo(0,0) }}
                  className={styles.pageNumBtn}>{totalPages}</button>
              </>
            )}
          </div>
        )}
